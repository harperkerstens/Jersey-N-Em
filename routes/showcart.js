...
router.get('/', function(req, res, next) {
    ...
    if (req.session.productList) {
        ...
        res.write("</table>");
        res.write("<h2>Edit Your Cart</h2>");
        
        res.write("<table><tr><th>Product Name</th><th>Quantity</th><th>Action</th></tr>");
        for (let i = 0; i < productList.length; i++) {
            product = productList[i];
            if (!product) {
            continue;
            }
            res.write("<tr><td>" + product.name + "</td>");
            res.write("<td><input type=\"number\" name=\"quantity_" + product.id + "\" value=\"" + product.quantity + "\" min=\"0\"></td>");
            res.write("<td><button type=\"submit\" name=\"update\" value=\"" + product.id.quantity + "\">Update</button>");
            res.write("<button type=\"submit\" name=\"remove\" value=\"" + product.id + "\">Remove</button></td></tr>");
        }
        res.write("</table>");
        res.write("</form>");
